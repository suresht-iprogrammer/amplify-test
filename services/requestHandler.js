...
export default class RequestHandler {
  ...
  static get(action, params = '') {
    return new Promise((resolve, reject) => {
      fetch(`${BaseUrl}${action}${params}`, RequestHandler.getHeader('get'))
  ...
  static post(action, data, isFile = false) {
    return new Promise((resolve, reject) => {
      fetch(`${BaseUrl}${action}`, RequestHandler.getHeader('post', data, isFile))
        .then(response => ({ promise: response.json(), status: response.status }))
        .then(({ promise, status }) => {
          promise.then((payload) => {
            resolve(RequestHandler.isSuccess(payload, status));
  ...
  static put(action, data, isFile = false) {
    return new Promise((resolve, reject) => {
      fetch(`${BaseUrl}${action}`, RequestHandler.getHeader('put', data, isFile))
        .then(response => ({ promise: response.json(), status: response.status }))
        .then(({ promise, status }) => {
          promise.then((payload) => {
            resolve(RequestHandler.isSuccess(payload, status));
  ...
  static delete(action) {
    ...
    return new Promise((resolve, reject) => {
      fetch(`${BaseUrl}${action}`, RequestHandler.getHeader('delete', {}))
        .then(response => ({ promise: response.json(), status: response.status }))
        .then(({ promise, status }) => {
          promise.then((payload) => {
            resolve(RequestHandler.isSuccess(payload, status));
